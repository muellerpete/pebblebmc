...
final class PebbleTuple {
	...
	/**
	 * The 'width' of the tuple's value; This value will always be 'NONE' for
	 * non-integer types.
	 */
	...
	private PebbleTuple(final int key, final TupleType type, final Width width,
			final int length, final Object value) {
		this.key = key;
		this.type = type;
		this.width = width;
		this.length = length;
	...
	static PebbleTuple create(final int key, final TupleType type,
			final Width width, final int value) {
		return new PebbleTuple(key, type, width, width.value,
				Long.valueOf(value));
	}

	static PebbleTuple create(final int key, final TupleType type,
			final Width width, final Object value) {
